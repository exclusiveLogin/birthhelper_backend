...
export class FeedbackEngine {
    ...
    feedbackReplyRateLimitCheck = async (userId: number, commentId: number, res: Response) => {
        const exist = await this.getLastRepliesByComment(commentId, userId).toPromise();
        if (exist) {
            // check dates
            res.status(429);
            res.statusMessage = 'Found review less than 5 days old. Please edit or delete it before posting a new one.';
    ...
    getLastRepliesByComment(parentCommentId: number, userId: number): Observable<Comment> {
        const addQ = " ORDER BY datetime_create DESC LIMIT 1";
        ...
        console.log('getLastFeedbackByUserAndTarget: ', q);
        return this.context.dbe
            .queryOnceOfList<Comment>(q).pipe();
    }
    ...
    async editReplyFeedbackComment(
        ...
        return await this.context.commentEngine.editComment(commentId, text, userId);
    }
        ...
        this.feedback.get("/listbycontragent/:contragentID", async (req, res) => {
            try {
                ...
                const summary: {
                    core: Feedback[];
                    slot: Feedback[]
                } = await this.getFeedbackListByContragent(contragentId, targetKey, sectionKey, statusKey);
