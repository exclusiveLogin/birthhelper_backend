...
export class ContainerEngine {
  ...
  async getContainers(
    ...
    for (const container of repo) {
      const qi = `SELECT \`${containerParams.db_links}\`.*, 
                        ...
                        ${containerParams.entity_fields
                          .map(
                            (f) => `\`${containerParams.db_entity}\`.\`${f}\``
                          )
                          .join(", ")} 
                        FROM \`${containerParams.db_links}\`
                        LEFT JOIN \`${containerParams.db_entity}\` 
                        ON \`${containerParams.db_links}\`.\`${
        containerParams.container_id_key
      }\` = \`${containerParams.db_entity}\`.\`id\`
                        WHERE \`${
                          containerParams.db_links
                        }\`.\`container_id\`=${container.id}`;

      const containerRecordList =
        (await this.context.dbe
          .queryList<ContainerRecordItem>(qi)
          .toPromise()) ?? [];
  ...
  async simpleSaveContainer(containerParams, id_container, ids) {
    ...
    if (containerParams && id_container && Array.isArray(ids)) {
      ...
      try {
        ...
        this.garbageHandler(
          [
            ...
            containerParams.db_entity,
            containerParams.entity_key,
            ...createSections,
          ],
          createSections
        );
      ...
      return Promise.reject({
        error:
          "Недостаточно входных данных для работы с контейнером(links_db && id_container && Array.isArray(ids))",
      });
  }
  ...
  async saveContainerHandler(req, res) {
    ...
    if (!!containers[req.params.name] && data.ids) {
      const containerParams = containers[req.params.name];
      const container_id = req.params.cid;

      try {
        const result = await this.simpleSaveContainer(
          containerParams,
          container_id,
          data.ids
        );
  ...
  async removeContainerItems(name, id) {
    ...
    if (containers[name]) {
      ...
      const qdd = `DELETE FROM \`${db_cont}\` WHERE container_id=${id}`;

      return this.context.dbe
        .queryList<null>(qdd)
        .pipe(
          tap((result) =>
            console.log("removeContainerItems result", qdd, result)
          ),
          mapTo(`Записи контейнеров с id = ${id} удалены`)
        )
        .toPromise();
  ...
  async getContainerHandler(req, res) {
    ...
    if (!req.params.name) {
      res.status(500);
      res.end(
        JSON.stringify({ error: "ошибка доступа : не указан контейнер" })
      );
    ...
    if (
      ...
      try {
        ...
        if (cid) {
          ...
          if (con) {
            ...
            res.status(500);
            res.send(
              JSON.stringify({ error: "Контейнер с таким ID не найден" })
            );
          }
        } else {
          res.send(containers);
      ...
      res.send(
        JSON.stringify({
          error: "ошибка доступа : Запрошенный контейнер не существует",
        })
      );
  ...
  garbageHandler(keys: string[], sections: SectionKeys[]): void {
    ...
    sections.forEach((k) =>
      this.context.searchEngine.resetSearchStoreBySection(k)
    );
    sections.forEach((k) =>
      this.context.searchEngine.resetSummaryStoreBySection(k)
    );
  }
