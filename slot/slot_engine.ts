...
export class SlotEngine {
  ...
  getContragentById(contragentID: number | string): Observable<Entity> {
    return this.context.entityEngine
      .getEntitiesByIds([+contragentID], "ent_contragents")
  ...
  async getSlotsByContragent(
    contragentID: number | string
  ): Promise<SectionedContragentSlots> {
    if (!contragentID) {
      throw new Error("Не передан ID контрагента");
    }
    ...
    await this.getContragentById(contragentID)
      .pipe(
        tap((contragent) => {
    ...
    const slotsKeys = sections
      ...
      .reduce(
        (acc, cur) => [...acc, ...cur.map((p) => p.entityKey)],
        [] as EntityKeys[]
      );
        ...
        tabs: config.tabs.map((t) => ({
          ...
          floors: t.floors.map((f) => ({
            ...
            list: f.consumerKeys
              ...
              .filter((_) => !!_)
              .map((consumer) => ({
                entKey: consumer.entityKey,
                restrictors: consumer.restrictors,
              }))
              .map((state) => {
                ...
                return data.filter((ent) =>
                  restrictors.length
                    ? restrictors.every((r) => ent._entity[r.key] === r.value)
                    : true
                );
              })
              .flat(),
  ...
  validator(data, name) {
    // все ли поля необходимые пришли на бек
